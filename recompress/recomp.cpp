...
unsigned int i;
   ...
   fseek(TheFile,0,0);
   fstat(_fileno(TheFile), &StatBuffer);
...
DWORD number;
   ...
   // commented out "press any key" message to avoid the converter pausing
   // the actual program has a method of skipping this, but that corrupts the graphic data slightly so this is necessary
